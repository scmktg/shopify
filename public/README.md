...
- `/public/watermark.png` → official WaterMark Certification Scheme mark, rendered on red by `components/product/WatermarkBadge.tsx`. Should be a **single-colour white-foreground** PNG (transparent background) so it sits on the red badge cleanly. Drop the official mark file here; do not invent or stylise the logo.
