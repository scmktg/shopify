...
function classifyRow(row, rules, resolves) {
...
async function main() {
  ...
  const classified = csv.map((row) => ({
    ...row,
    ...classifyRow(row, rules, resolves),
  }));
  ...
  for (const r of classified) {
    ...
    if (r.bucket === 'covered_explicit' || r.bucket === 'covered_fallback' || r.bucket === 'no_redirect_needed') {
...
function renderProposals(rows, existingRules, resolves) {
  ...
  out.push('// covered by next.config.js. Review each before merging — many are');
  out.push('// flagged needs_manual_review and have no destination set.');
  out.push('//');
  out.push('// Categories:');
  out.push('//   gone_410             — emit { source, destination: false } in next.config.js');
  out.push('//                          (the `false` form returns a 410 via Next).');
